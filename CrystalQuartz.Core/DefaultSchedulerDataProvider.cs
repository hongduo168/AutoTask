namespace CrystalQuartz.Core
{
    ...
    public class DefaultSchedulerDataProvider : ISchedulerDataProvider
    {
        ...
        public List<SchedulerData> Data
        {
            get
            {
                List<SchedulerData> result = new List<SchedulerData>();
                foreach (var scheduler in _schedulerProvider.Scheduler)
                {

                    var metadata = scheduler.GetMetaData();

                    result.Add(new SchedulerData
                               {
        ...
        public TriggerData GetTriggerData(TriggerKey key)
        {
            foreach (var scheduler in _schedulerProvider.Scheduler)
            {
                //var scheduler = _schedulerProvider.Scheduler.First();
                if (scheduler.IsShutdown)
                {
                    //return null;
                    continue;
